...
@app.route('/generate-questions', methods=['POST'])
def generate_questions():
    global question_pool, cache_time
    try:
        now = time.time()
        if not question_pool or (now - cache_time) > CACHE_DURATION:
            question_pool = []
            for _ in range(3):
                question_pool.append(generate_fresh_questions())
            cache_time = now

        questions = random.choice(question_pool).copy()
